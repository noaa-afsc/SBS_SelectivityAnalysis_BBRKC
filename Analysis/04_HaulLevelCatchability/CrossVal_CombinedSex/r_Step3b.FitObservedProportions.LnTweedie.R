...
if (FALSE){
  ...
  #--the above creates fold_x.RData and log_x.txt files in the working directory. 
  ##--Make sure to move these to "ModelResults_LnTweedieModels" subfolder before further processing.
}
...
if (FALSE){
  ...
  p1 = ggplot(dfrScrs |> dplyr::filter(smths %in% sel_mdls) |> 
         ...
         labs(y="GCV score",subtitle="log-Tweedie models") + 
         wtsPlots::getStdTheme() + 
         theme(axis.text.x=element_text(size=12,angle=345,hjust=0),
               axis.title.x=element_blank());
  ggsave(file.path(dirThs,"pltBestModels.All.LnTweedie.pdf"),plot=p1,width=6.5,height=4);
  
  #--evaluate best model
  best_smth = "s(z)";#--user must determine this based on results above
...
if (FALSE){
  ...
  dfrPrd = prdMod(mdl=best_mdl,trms=c("(Intercept)","s(z)"),type="link",lst=grdPrd,p=0.10) |> 
            dplyr::mutate(emp_sel=exp(emp_sel),
                          lci=exp(lci),
                          uci=exp(uci));
  plts2 = plotMod(dfrPrd,ylims=c(0,3));
  dfrPrd = prdMod(mdl=best_mdl,trms=c("all"),type="link",lst=grdPrd,p=0.10) |> 
            dplyr::mutate(emp_sel=exp(emp_sel),
                          lci=exp(lci),
                          uci=exp(uci));
  plts3 = plotMod(dfrPrd,ylims=c(0,3));
}
...
if (FALSE){
  ...
  lstPrd = list();
  for (sel_mdl in sel_mdls[!is.na(sel_mdls)]){
    lstPrd[[sel_mdl]]  = prdMod(lstMdls[[sel_mdl]],trms=c("all"),type="link",lst=grdPrd,p=0.10) |> 
  ...
  ggplot(dfrPrd,aes(x=z,y=emp_sel,ymin=lci,ymax=uci,colour=trms,fill=trms)) + 
    ...
    scale_y_continuous(limits=c(0,3.0),oob=scales::squish) + 
